...
export default function BottomNav({ locale }: BottomNavProps) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-brand-cream-dark safe-area-pb md:hidden">
        ...
        {links.map((link) => (
          <Link
            ...
            className={cn(
              ...
              isActive(link.href) ? "text-brand-brown" : "text-gray-400"
            )}
          >
            <span className="relative">
              {link.icon}
              {link.badge ? (
                <span className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-brand-gold text-brand-brown text-[10px] font-bold rounded-full flex items-center justify-center">
